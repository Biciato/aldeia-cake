<?php if(count($turmas_servico) > 0)
  {
    ...
    foreach($turmas_servico as $turmas)
      {
        ?>
        <div class="row form-group turmas-<?php echo $key; ?>"  data-parent-id="atendimento-<?php echo $key; ?>">
            <div class="col-sm-12">
                ...
                <select class="form-control" name="turmas[<?php echo $turmas['servico']->ServicoAux->id; ?>]" id="turmas-<?php echo $key; ?>-<?php echo $turmas['servico']->id; ?>">
                    ...
                    <?php foreach($turmas['turmas'] as $turma)
                      {
                        ?>
                        <option <?php echo (@$aluno->turmas_array[$turmas['servico']->ServicoAux->id] == $turma->id) ? 'selected="selected"' : ''; ?> value="<?php echo $turma->id; ?>"><?php echo $turma->nome; ?></option>
                        <?php
                      } ?>
                </select>
            </div>
